...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/others/papi_best_set_old.c $
 | @last_commit: $Date: 2015-03-03 14:33:11 +0100 (Tue, 03 Mar 2015) $
 | @version:     $Revision: 3127 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: papi_best_set_old.c 3127 2015-03-03 13:33:11Z harald $";
