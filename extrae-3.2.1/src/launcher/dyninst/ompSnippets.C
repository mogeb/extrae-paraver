...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/launcher/dyninst/ompSnippets.C $
 | @last_commit: $Date: 2013-05-13 18:42:21 +0200 (Mon, 13 May 2013) $
 | @version:     $Revision: 1722 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: ompSnippets.C 1722 2013-05-13 16:42:21Z harald $";
