...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/launcher/dyninst/mpiSnippets.C $
 | @last_commit: $Date: 2014-06-18 12:53:30 +0200 (Wed, 18 Jun 2014) $
 | @version:     $Revision: 2760 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: mpiSnippets.C 2760 2014-06-18 10:53:30Z harald $";
