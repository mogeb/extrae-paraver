...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/launcher/dyninst/cudaSnippets.C $
 | @last_commit: $Date: 2012-10-23 14:23:25 +0200 (Tue, 23 Oct 2012) $
 | @version:     $Revision: 1296 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: cudaSnippets.C 1296 2012-10-23 12:23:25Z harald $";
