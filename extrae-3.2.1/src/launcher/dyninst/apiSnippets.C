...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/launcher/dyninst/apiSnippets.C $
 | @last_commit: $Date: 2012-10-23 11:36:58 +0200 (Tue, 23 Oct 2012) $
 | @version:     $Revision: 1293 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: apiSnippets.C 1293 2012-10-23 09:36:58Z harald $";
