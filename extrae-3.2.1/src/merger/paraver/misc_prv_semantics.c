...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/merger/paraver/misc_prv_semantics.c $
 | @last_commit: $Date: 2015-10-28 17:34:45 +0100 (Wed, 28 Oct 2015) $
 | @version:     $Revision: 3507 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: misc_prv_semantics.c 3507 2015-10-28 16:34:45Z harald $";
