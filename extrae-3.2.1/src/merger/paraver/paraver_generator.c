...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/merger/paraver/paraver_generator.c $
 | @last_commit: $Date: 2015-10-06 18:54:49 +0200 (Tue, 06 Oct 2015) $
 | @version:     $Revision: 3428 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: paraver_generator.c 3428 2015-10-06 16:54:49Z harald $";
