...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/merger/dimemas/mpi_trf_semantics.c $
 | @last_commit: $Date: 2012-10-25 13:05:07 +0200 (Thu, 25 Oct 2012) $
 | @version:     $Revision: 1311 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: mpi_trf_semantics.c 1311 2012-10-25 11:05:07Z harald $";
