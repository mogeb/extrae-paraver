...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/clocks/bgq_clock.c $
 | @last_commit: $Date: 2012-01-27 13:07:10 +0100 (Fri, 27 Jan 2012) $
 | @version:     $Revision: 947 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: bgq_clock.c 947 2012-01-27 12:07:10Z harald $";
