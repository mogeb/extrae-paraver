...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/interfaces/MPI/mpi_interface.c $
 | @last_commit: $Date: 2015-10-08 12:20:41 +0200 (Thu, 08 Oct 2015) $
 | @version:     $Revision: 3443 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: mpi_interface.c 3443 2015-10-08 10:20:41Z harald $";
