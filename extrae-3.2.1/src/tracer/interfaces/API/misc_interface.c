...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/interfaces/API/misc_interface.c $
 | @last_commit: $Date: 2015-04-22 15:23:11 +0200 (Wed, 22 Apr 2015) $
 | @version:     $Revision: 3265 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: misc_interface.c 3265 2015-04-22 13:23:11Z harald $";
