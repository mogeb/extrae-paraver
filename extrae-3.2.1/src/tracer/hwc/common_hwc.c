...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/hwc/common_hwc.c $
 | @last_commit: $Date: 2015-07-20 17:12:25 +0200 (Mon, 20 Jul 2015) $
 | @version:     $Revision: 3391 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: common_hwc.c 3391 2015-07-20 15:12:25Z harald $";
