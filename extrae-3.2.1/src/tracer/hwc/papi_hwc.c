...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/hwc/papi_hwc.c $
 | @last_commit: $Date: 2015-04-22 16:43:20 +0200 (Wed, 22 Apr 2015) $
 | @version:     $Revision: 3266 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: papi_hwc.c 3266 2015-04-22 14:43:20Z harald $";
