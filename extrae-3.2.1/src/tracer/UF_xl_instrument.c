...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/UF_xl_instrument.c $
 | @last_commit: $Date: 2013-06-19 17:35:42 +0200 (Wed, 19 Jun 2013) $
 | @version:     $Revision: 1849 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: UF_xl_instrument.c 1849 2013-06-19 15:35:42Z harald $";
