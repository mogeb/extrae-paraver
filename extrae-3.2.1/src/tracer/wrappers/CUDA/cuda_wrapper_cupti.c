...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/wrappers/CUDA/cuda_wrapper_cupti.c $
 | @last_commit: $Date: 2015-02-25 16:25:28 +0100 (Wed, 25 Feb 2015) $
 | @version:     $Revision: 3124 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: cuda_wrapper_cupti.c 3124 2015-02-25 15:25:28Z harald $";
