...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/wrappers/OMP/gnu-libgomp-4.2.c $
 | @last_commit: $Date: 2015-06-05 17:18:47 +0200 (Fri, 05 Jun 2015) $
 | @version:     $Revision: 3354 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: gnu-libgomp-4.2.c 3354 2015-06-05 15:18:47Z harald $";
