...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/wrappers/OMP/ibm-xlsmp-1.6.c $
 | @last_commit: $Date: 2015-04-21 16:45:09 +0200 (Tue, 21 Apr 2015) $
 | @version:     $Revision: 3260 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: ibm-xlsmp-1.6.c 3260 2015-04-21 14:45:09Z harald $";
