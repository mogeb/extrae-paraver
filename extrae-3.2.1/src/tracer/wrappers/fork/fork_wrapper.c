...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/wrappers/fork/fork_wrapper.c $
 | @last_commit: $Date: 2015-06-23 10:04:50 +0200 (Tue, 23 Jun 2015) $
 | @version:     $Revision: 3364 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: fork_wrapper.c 3364 2015-06-23 08:04:50Z harald $";
