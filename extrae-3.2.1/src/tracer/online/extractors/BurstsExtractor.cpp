...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/online/extractors/BurstsExtractor.cpp $
 | @last_commit: $Date: 2014-09-08 12:11:01 +0200 (Mon, 08 Sep 2014) $
 | @version:     $Revision: 2955 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */

#include "common.h"

static char UNUSED rcsid[] = "$Id: BurstsExtractor.cpp 2955 2014-09-08 10:11:01Z gllort $";
