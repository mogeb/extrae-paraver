...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/online/extractors/Signal.cpp $
 | @last_commit: $Date: 2013-03-08 15:26:30 +0100 (Fri, 08 Mar 2013) $
 | @version:     $Revision: 1583 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */

#include "common.h"

static char UNUSED rcsid[] = "$Id: Signal.cpp 1583 2013-03-08 14:26:30Z harald $";
