...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/online/SpectralFilter.cpp $
 | @last_commit: $Date: 2013-09-06 10:29:34 +0200 (Fri, 06 Sep 2013) $
 | @version:     $Revision: 2094 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */

#include "common.h"

static char UNUSED rcsid[] = "$Id: SpectralFilter.cpp 2094 2013-09-06 08:29:34Z gllort $";
