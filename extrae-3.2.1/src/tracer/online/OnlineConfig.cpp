...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/tracer/online/OnlineConfig.cpp $
 | @last_commit: $Date: 2014-10-16 13:36:20 +0200 (Thu, 16 Oct 2014) $
 | @version:     $Revision: 3012 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */

#include "common.h"

static char UNUSED rcsid[] = "$Id: OnlineConfig.cpp 3012 2014-10-16 11:36:20Z gllort $";
