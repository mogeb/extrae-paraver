...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/tests/cross-compile-sizeof/check-sizeofs.c $
 | @last_commit: $Date: 2012-10-11 10:49:36 +0200 (Thu, 11 Oct 2012) $
 | @version:     $Revision: 1249 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
static char rcsid[] = "$Id: check-sizeofs.c 1249 2012-10-11 08:49:36Z harald $";
